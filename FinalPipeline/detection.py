...
def draw_ball_position(frame, court_detector, xy, i):
    """
    Calculate the ball position of both players using the inverse transformation of the court and the x, y positions.
    Marks the ball position as green if bounce == 1, and red otherwise.
    """
    inv_mats = court_detector.game_warp_matrix[i]
    img = frame.copy()

    # Ball locations
    if not xy.empty and xy.iloc[i] is not None:
        coord = xy.iloc[i]
        p = np.array([coord['x'], coord['y']], dtype='float64')
        ball_pos = np.array([p[0].item(), p[1].item()]).reshape((1, 1, 2))
        transformed = cv2.perspectiveTransform(ball_pos, inv_mats)[0][0].astype('int64')
        
        # Determine color based on bounce value
        color = (0, 255, 0) if coord['bounce'] else (0, 0, 255)
        
        cv2.circle(img, (transformed[0], transformed[1]), 35, color, -1)
...
def create_top_view(court_detector, detection_model, xy, fps):
    ...
    out = cv2.VideoWriter('FinalPipeline/vout/minimap.mp4',cv2.VideoWriter_fourcc('X', 'V', 'I', 'D'), fps, (v_width, v_height))
    ...
    for feet_pos_1, feet_pos_2 in zip(smoothed_1, smoothed_2):
        ...
        frame=draw_ball_position(frame, court_detector, coords, i) 
...
def interpolation(coords):
    ...
    # Handle NaNs in x coordinates
    nans_x, idx_x = nan_helper(x)
    if nans_x.any():
        x[nans_x] = np.interp(idx_x(nans_x), idx_x(~nans_x), x[~nans_x])

    # Handle NaNs in y coordinates
    nans_y, idx_y = nan_helper(y)
    if nans_y.any():
        y[nans_y] = np.interp(idx_y(nans_y), idx_y(~nans_y), y[~nans_y])
...
def remove_outliers(x, y, coords):
    # Find indices where both x and y are greater than 50
    ids = set(np.where(x > 50)[0]) & set(np.where(y > 50)[0])
    
    for id in ids:
        # Extract neighboring coordinates and handle boundary conditions
        left = coords.iloc[id-1] if id-1 >= 0 else pd.Series({'x': 0, 'y': 0, 'bounce': False})
        middle = coords.iloc[id]
        right = coords.iloc[id+1] if id+1 < len(coords) else pd.Series({'x': 0, 'y': 0, 'bounce': False})
        
        # Replace None values with [0, 0] for x and y
        if pd.isnull(left['x']):
            left['x'] = 0
        if pd.isnull(left['y']):
            left['y'] = 0
        if pd.isnull(right['x']):
            right['x'] = 0
        if pd.isnull(right['y']):
            right['y'] = 0
        if pd.isnull(middle['x']):
            middle['x'] = 0
        if pd.isnull(middle['y']):
            middle['y'] = 0
        
        # Find the maximum of the three points
        max_coord = max(
        ...
        if max_coord != (0, 0):
            ...
            # Update the DataFrame by setting the outlier coordinate to NaN
            coords.loc[(coords['x'] == max_x) & (coords['y'] == max_y), ['x', 'y']] = np.nan
